...
### Added
- Host restart recovery: leftover parked asks stay answerable; running or takeover leftovers show a check / new-attempt card instead of a silent failed run (`0037_run_waits.sql`, `POST /v1/threads/{bot_id}/recovery`). Queued owner jobs are not replayed. A late token from the dying process cannot complete a recovered run.
- Owner Send carries a client `command_id` minted before the request (`0038_owner_commands.sql`). A lost HTTP response retries the same id and does not create a second run or user bubble. A changed payload with that id is 409. Timeout after the host run exists is `unknown` (`execution_state: unconfirmed`), not failed-with-retry, and a new attempt gets a new id linked with `parent_command_id`.
