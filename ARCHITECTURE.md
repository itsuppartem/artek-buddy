...
| State | Where |
| --- | --- |
| Threads, bots, devices, pairing hashes, memory book, routines, consent, artifacts, audit chain, durable jobs, activity log, search documents, automations, per-turn token usage, restart wait records, owner send command ids | Postgres (`HistoryStore`, 38 SQL files under `src/artek_buddy/db/migrations/`). Host API and worker both call `apply_migrations` on boot; a session `pg_advisory_lock` serializes them. Each applied file stores a sha256; a rewritten historical file fails the run. The `activity` table is a workspace-monotonic sequence for resumable SSE; `EventHub` is live fan-out only and is not durability. The `search_documents` projection is ranked only after an authorized `resource_id` filter. Cron routines keep firing; each fire is a versioned `automation_runs` row keyed by an idempotency key. Token usage rows are keyed by bot/run (`GET /v1/usage`, `GET /v1/usage/summary`) and store an estimated USD (integer micros) when the host knows the model. Fast for that estimate is bound when the run starts (`0036_run_usage_fast.sql`), not from the live Models default at persist time. |
