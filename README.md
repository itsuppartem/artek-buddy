...
**Usual path:** a GitHub Release. A merge into `main` that bumps `VERSION` attaches `artek-buddy-client_<version>_all.deb` (no baked host URL), `SHA256SUMS`, CycloneDX SBOMs, and `install-host.sh` only after `test` and CodeQL on **that commit** are green and `release.yml` is dispatched from that `main` SHA (not a `workflow_run` loaded from `develop`). Dispatch refuses an existing VERSION tag or GitHub Release (`force` cannot skip). The Release tag is that SHA, peeled before GHCR `latest` moves. The host image is scanned before `latest` moves. GitHub Releases are not pruned by that workflow.
