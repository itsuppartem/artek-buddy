...
- **Use a bot-specific command token without putting it in chat or the desktop.** Save GitHub, PyPI, or a named token in that bot's Settings. A worker asks Allow once / Always, then the supervisor runs one command in a disposable container with only that bot's home and token mapping. The container is removed afterward; returned output is bounded and redacted at the supervisor and broker.
