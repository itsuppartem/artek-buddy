...
- **Retain useful context from chat.** Talk normally — name, tone, paths, a standing ban. The bot writes those into a book (no profile form). A later fact revises the chapter instead of stacking a contradiction. The next turn already has the owner book and this chat's standing rules. Work notes come back when the turn is about that work. The Memory panel can show the same book. Replayable skills are later.
