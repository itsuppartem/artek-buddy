...
| Phone (width ≤720, or a short landscape phone such as 812×375) | host page or a narrow window | One column. A wide window (1280×720 and up) stays the three-column `.deb` shell. Bottom `phone-nav`: Chats / Chat / Desktop (`phone-tab-*`). Tap targets 44px. `safe-area-inset-top` clears the notch; the nav and the shell share one bottom inset (no second empty strip). Inbox list fills the space between Search and Plugins / Models so bot rows stay on screen. Create / Models / Plugins open Desktop. Close on the Desktop pane or the fullscreen ✕ returns to Chat. Pad / Keyboard exist only on the fullscreen desktop (`computer-overlay` `data-phone-desk`): drag moves the pointer, tap left click, two fingers right click. Keyboard extras (Esc / Tab / Enter / Bksp / Del / arrows) overlay the screen and do not shrink the pad, so the tan cursor stays on the same desktop pixel. Typing uses the phone keyboard; there is no visible type field (the guest already shows the text). The phone’s Done checkmark dismisses that keyboard. Chats and Chat stay the ordinary list and thread. `.deb` wide overlay stays the mouse iframe. iPhone Share → Add to Home Screen (`home-screen-hint`) sits at the top of the shell (`phone-host-banners`), not over Models / composer / the nav; Got it dismisses it. `turn-on-alerts` asks for web notifications after a tap; iOS only exposes that from the home-screen icon. No This-PC Allow on the phone filesystem. |
