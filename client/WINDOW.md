...
| Phone (≤720px on the short side) | host page or a narrow window | One column in portrait and landscape (iPhone 11 Pro 375×812 / 812×375). Bottom `phone-nav`: Chats / Chat / Desktop (`phone-tab-*`). Tap targets 44px. `safe-area-inset-bottom`. Create / Models / Plugins open Desktop. Fullscreen desktop (`computer-overlay` `data-phone-desk`) is a pad: drag moves the pointer, tap left click, two fingers right click. **Keyboard** opens the phone keyboard plus Esc/Tab/arrows. Chrome is compact (Team / You have control). `.deb` wide overlay stays the mouse iframe. iPhone Share → Add to Home Screen (`home-screen-hint`) sits at the top of the shell (`phone-host-banners`), not over Models / composer / the nav; Got it dismisses it. `turn-on-alerts` asks for web notifications after a tap; iOS only exposes that from the home-screen icon. No This-PC Allow on the phone filesystem. |
