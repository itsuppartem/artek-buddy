...
Reasoning and Fast persist for the next send without picking a different chip. They say what they change. There is no second Save next to Reasoning; **Use this model** is the commit when that row has a list. An empty provider names the next step (`models-empty-{id}`: paste a key) and does not offer a dead Use this model. The in-use chip is tan, bold, and ringed. The open chat gets a `meta-block`: `Using {id} · {effort} · Fast.` A live turn is not cancelled; that line ends with `This turn keeps going.` The next send starts a new model session.
...
Allow once / Always / Deny for browse, page, and owner_*. A read-only owner job marked `auto` runs without a card (`/local/owner-*`) on the Linux `.deb`. Before touching This PC the desktop client ACK-claims that queued job; reconnect and a second window do not execute an acknowledged or completed job again. Older clients may still post a result without ACK. After Allow the desktop client reads, writes, lists, or execs under `$HOME` and posts the result. Paths outside `$HOME` are 403 and stay on the consent card. The host phone page never runs owner tools (`/local/owner-*` is 403) and leaves auto jobs for the paired Linux client instead of posting a failure for them. Composer Stop is `thread-stop` (`aria-label="Stop"`). Settings is the Settings button, not the thread title.
