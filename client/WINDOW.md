...
Auth error (`auth-error`), including a 401/403 on workspace `/v1/events` while thread REST still works: **Pair this computer again** (`.deb`) or **Pair this phone again** (host page) → `unpair` → pairing. The window does not stay quietly paired when background needs-you stops.
