...
- The selected row, thread header, composer, and computer pane always name the same bot. Unsent text, files, undo, Reply, and Send belong to that chat: a switch does not carry them, and a late send from another chat does not disable Send or restore files here. The thread never blanks to an empty column on a switch.
