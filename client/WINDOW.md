...
Settings: the same fields plus Instructions (standing orders, not labelled Prompt), mode change (rebinds the desktop; home is not copied), Restart / Stop / Reset (Reset wipes that home; Team reset wipes the shared desktop), busy-bot name, notifyOnFinish, Delete with optional purge memories. **Edit profile** opens the fields. Save flashes **Saved** for about a second, then the sheet. A host error stays under the row.
