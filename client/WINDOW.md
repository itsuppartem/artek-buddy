...
On the `.deb` surface, a reply / failure with `notifyOnFinish` enabled and every owner question / takeover calls loopback `notify` once when the window is hidden or another chat is open. The open chat stays quiet while it is still on screen — looking at the OS notification list (window blur) does not count as leaving that thread and must not stack more native alerts or mark the chat read. Read / dismiss happen only when that thread is focused on screen, or the owner opens it / taps Dismiss. One native row per bot (same tag replaces the previous). Memory `Remembered:` / `Forgot:` lines and auto This-PC jobs are not attention. The host page uses the browser notification path instead and never calls the Linux loopback endpoint. Native alerts are posted from the running GTK process (libnotify) so GNOME keeps an **Artek Buddy** row in the notification list; they use the installed desktop-entry identity and packaged icon. The tray / dock badge is a separate urgency hint.
