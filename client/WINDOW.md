...
| Phone (≤720px) | host page or a narrow window | One column. Sized for iPhone 11 Pro (375×812 home-screen). Bottom `phone-nav`: Chats / Chat / Desktop (`phone-tab-*`). Tap targets 44px. `safe-area-inset-bottom`. Create / Models / Plugins open Desktop. iPhone Share → Add to Home Screen (`home-screen-hint`). `turn-on-alerts` asks for web notifications after a tap; iOS only exposes that from the home-screen icon. No This-PC Allow on the phone filesystem. |
