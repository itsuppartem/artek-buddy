...
| | |
| --- | --- |
| `main` | Release **0.2.0** (after this bump lands). |
| `develop` | Same generation as 0.2.0 until the next VERSION bump. |
...
| GitHub Release `.deb` | **0.2.0** once the Release exists. Until then, build from this tree. |
...
| Open a chat, go to **Today**, then let `please e2e-slow` finish: that helper stays unread (Deb Chats attention count; Phone Chats unread pin). Same if you open Library, Bot profile, or phone Desktop instead of Today. Opening the thread on screen then clears unread | [ ] | [ ] |
...
| Switching the model chip (composer / grok) does not turn Fast back on. After Fast is off, a Send plus a spawned worker must not add Fast usage rows. Stop kills a running worker | [ ] | [ ] |
...
| While a grok-4.6 turn is running, uncheck Fast. After it finishes, Work log $ for **that** run still matches Fast (about 2× Standard), not the new default. Inverse: Fast off at Send, then check Fast on while it runs — $ stays Standard. Scripted hosts usually have counts without $ | [ ] | — |
...
| Deb and the host URL in a desktop browser: copy `host clipboard текст` (and ASCII URLs), focus a guest field, then Ctrl+V. The full text appears once in the guest (no file picker dialog) and populates the guest clipboard; right-click in Chromium shows Paste active | [ ] | — |
...
| Scripted `please e2e-worker-progress`: on Deb, use the installed GTK/WebKit window (not browser `--serve`). The first **Still working:** line appears promptly in the waiting slot under the thread only (`typing-indicator`); the header stays **Working** and `work-summary` does not repeat that sentence. Message stays usable; **no** worker card. The second scripted progress line replaces that same slot. Then exactly one **The background job is done.** appears and the slot is gone | [ ] | [ ] |
| Scripted `please e2e-worker-essay`: after that clipped **Still working:** step, the worker streams a long finish-body essay. The waiting slot (and Work log **Latest work**) keeps the clipped `report_progress` line or dots for the whole delay — never the essay. Worker cards stay hidden; `typing-indicator` stays up while the worker runs. Then exactly one **The background job is done.** and the slot is gone | [ ] | [ ] |
| Two scripted worker turns in one chat (`please e2e-background-worker-chat` twice): after the second completes, **Show work log** is still there. Reopen (and after close / switch-away-and-back) lists **both** run groups; the first worker task is still under the older group. **Latest work** is not blank just because in-flight progress is gone. Each group can show in/out/total and an estimated $ from `GET /v1/usage` when the host has them; missing usage is omitted. Scripted hosts usually have counts without $. A live grok-4.6 turn should show the $ next to the counts. No Models spend screen. Collapsed worker rows stay short | [ ] | [ ] |
...
| Scripted `please e2e-worker-unclear-assignment`: the worker Ask card asks which artifact to build (not a guessed finish). Answering it resumes **that same worker** | [ ] | [ ] |
...
| Opening a chat **on screen** in the focused window marks it read. A selected helper on Today / Library / settings / phone Desktop is not on screen: a new reply stays unread and keeps notify. Looking at or dismissing an OS notification does **not** mark read. A reply while that thread is actually visible does not leave the unread pin | [ ] | [ ] |
...
| `please e2e-unclear-assignment`: first vague assignment parks on an Ask card (Needs you, not Ready / Task is complete). Answering continues the **same run**. `please e2e-specified-assignment` finishes with **no** Ask card | [ ] | [ ] |
| Restart the host while an Ask / Allow card is waiting: the thread is not a failed run. The same card stays answerable (safe to continue). Restart during `please e2e-slow` or a takeover: a **recovery-card** (check or start a new attempt — no Continue on check). A queued This-PC job is not executed twice | [ ] | [ ] |
...
| After Send: **Still delivering this send.** (`send-delivering`) until the host accepts — no typing dots and no running helper yet. `please e2e-unknown-timeout` becomes **Checking** / `run-unknown`, not a `run-error` Send-again. A lost HTTP response after Send is the same unknown card, not a second user bubble and not the offline pending mark. Two windows (or a retry) of the same in-flight command still one bubble and one run. Before Models has a key, retrying the same `command_id` is still one user bubble; after setup, the same id starts that send once. A Send queued behind a live turn retries as still waiting, not the previous turn's answer; after that turn finishes, retry follows the follow-up run. Stop still works. A new Send is a new command id | [ ] | [ ] |
...
| `please e2e-unclear-assignment`: vague first assignment asks; the answer continues the same work. `please e2e-specified-assignment` does not ask. Live Cursor: send an outcome that names no file/site/person — one Ask card, not a guessed result | [ ] | [ ] |
...
11. **§28 Save feedback**
12. **§10 workers** `please e2e-background-worker-chat` then status; the acknowledgement is immediate and does not start a second plan; Message stays usable; one final result. Send that prompt twice: Work log keeps **both** run groups and usage (tokens and $ when the host has them). `please e2e-worker-progress`: **Still working:** only in the waiting slot (header is Working; work-summary does not repeat it), not a durable chat bubble, and no worker card. `please e2e-worker-essay`: the slot keeps that clipped line (or dots) through the long finish body; never the essay. `please e2e-worker-activity-no-text` then status keeps the same worker. `please e2e-lead-owner-ssh` finishes; the next Send is a new run. `please e2e-worker-auto-read`: refresh still lists the queued This-PC read; ACK once. `please e2e-worker-blocked-browser` then answer the Ask card; `please e2e-worker-unclear-assignment` then answer the worker Ask card (same worker); `please e2e-worker-park-takeover` then Release. **This-PC** back-to-back read/list and one Allow action exactly once on Deb (including a git/find write form: card, Deny leaves the file/branch uncreated)
...
14. **§14 Inbox** Search empty + one click opens that row. A unique phrase in a transcript appears under Search as a host hit and opens that chat. Switch A↔B keeps messages (no empty flash); a first/uncached visit shows Loading this chat…. Open A → Today → `please e2e-slow` finishes: A stays unread until you open that thread on screen
15. **§15 Composer + links** Ctrl+A selects, does not Send; `please e2e-markdown-preview` opens and copies its URL. Connection **Open to connect** is the same kind of link. `please e2e-send-terminal`: one complete answer, no paraphrased second bubble. `please e2e-send-then-answer`: teaser then the spec, both stay. `please e2e-unclear-assignment` asks; answering continues the same run; `please e2e-specified-assignment` does not ask. Stop / typing dots clear and Message is usable. Unsent draft stays on that chat after a switch. Restart-during-wait: parked Ask stays; running/takeover shows a recovery card, not a failed run. `please e2e-unknown-timeout`: Checking / `run-unknown`, not Send-again; delivering until the host accepts, no typing dots before accept
...
18. **§15 Stop** on a live turn shows Stopped. A late complete does not land. The next Send is a new run and does not show the busy / already-has-active-run error
