...
| After Send: **Still delivering this send.** (`send-delivering`) until the host accepts — no typing dots and no running helper yet. `please e2e-unknown-timeout` becomes **Checking** / `run-unknown`, not a `run-error` Send-again. A lost HTTP response after Send is the same unknown card, not a second user bubble and not the offline pending mark. Stop still works. A new Send is a new command id | [ ] | [ ] |
