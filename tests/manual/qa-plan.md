...
| `please e2e-send-then-answer`: a short teaser then a different final answer, both in order after reload. Stop / typing dots are gone. `please e2e-send-then-repeat` stays one bubble | [ ] | [ ] |
...
| Tokens: GitHub, PyPI, or **Another token** (name + secret) for **this bot**. Store then `••••` + last four. Another bot's Settings does not show that token. Forget empties the row. Reset / Team ↔ Private keep it. A named token in Message is stored here and stripped from the bubble | [ ] | [ ] |
...
12. **§1 / §21 Escape** on Settings and New bot. **§21 Tokens**: Store a named token last four on one bot; the other bot is empty; a named token in Message is stripped from the bubble and shows last four in Settings
