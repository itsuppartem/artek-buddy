...
| In `please e2e-worker-progress` / `please e2e-worker-essay`, the compact summary shows the clipped **Still working:** step (never the worker finish body). There is exactly one **Show work log** control. Worker briefs are collapsed, scannable rows—not a full prompt wall—and detail is not duplicated as durable chat messages | [ ] | [ ] |
...
19. **§15 dead wait** after 60+ minutes idle, the first live Send completes without a red line; scripted `please e2e-dead-wait` completes on that send and `please e2e-dead-wait-stuck` still shows one terminal error that the host retried
