...
def test_computer_pane_start_and_close(page: Page, client_url: str, host_url: str) -> None:
    ...
    open_computer_pane(page)
    expect(page.get_by_test_id("computer-label")).to_contain_text(f"{name}\u2019s computer")
    ...
    expect(page.get_by_text("Running", exact=True).or_(page.get_by_test_id("computer-running"))).to_be_visible(
        timeout=8_000
    )
