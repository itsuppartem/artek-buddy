...
def test_failed_banner_on_other_chat(page: Page, client_url: str, host_url: str) -> None:
    ...
    box.press("Enter")
    open_chat(page, watcher)
    expect(page.get_by_test_id("attention-alert")).to_contain_text(f"{speaker} failed", timeout=15_000)
    expect(page.get_by_test_id("attention-alert")).to_have_count(0, timeout=8_000)
