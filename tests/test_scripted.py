...
class ScriptedRuntimeTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prompt_scenarios_and_delay(self) -> None:
        ...
        send_file = steps_for_prompt("e2e-send-file")
        self.assertEqual(send_file[0].tool, "send_file")
        self.assertEqual(send_file[0].args.get("path"), "notes.txt")
        missing = steps_for_prompt("e2e-send-file-missing")
        self.assertEqual(missing[0].tool, "send_file")
        self.assertEqual(missing[0].args.get("path"), "missing-notes.txt")
        page = steps_for_prompt("e2e-consent-page")
        self.assertIsNotNone(page[0].consent)
        self.assertEqual((page[0].consent or {})["action_class"], "page_input")
        self.assertIn("Fill, type, or click", (page[0].consent or {})["summary"])
        browse = steps_for_prompt("e2e-consent-browse")
        self.assertEqual((browse[0].consent or {})["action_class"], "browse")
        exec_long = steps_for_prompt("e2e-consent-exec-long")
        self.assertEqual((exec_long[0].consent or {})["action_class"], "owner_exec")
        self.assertIn("Изображения", (exec_long[0].consent or {})["detail"] or "")
