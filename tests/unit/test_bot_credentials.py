...
def test_detects_github_and_pypi_shapes() -> None:
...
def test_rejects_wrong_provider_shape(tmp_path: Path) -> None:
    vault = BotCredentialStore(tmp_path)
    with pytest.raises(ValueError, match="secret does not match"):
        vault.put(BOT_A, "github", PYPI_FIXTURE)
    with pytest.raises(ValueError, match="unknown provider"):
        vault.put(BOT_A, "aws", GITHUB_FIXTURE)
