...
| Owner job delivery | A stale id, reconnect, or two windows executes the wrong write/command twice | Host ↔ paired `.deb` | Request ids stay on the current call stack; a new client atomically ACK-claims queued work; Postgres stores lifecycle but not command/file content; terminal jobs reject late results | An old client that does not ACK remains compatible and cannot prevent two old windows from performing the same side effect. Host restart loses the in-memory payload and the run fails instead of replaying it. |
