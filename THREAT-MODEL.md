...
| GHCR / Release `.deb` | Supply-chain swap | GitHub + Actions | Release only after green `test` on that `main` SHA; `SHA256SUMS`; CycloneDX SBOM; GitHub Artifact Attestations; Actions pinned by commit SHA; CodeQL / pip-audit / Trivy | pytest-only pip-audit ignores (not in the host image) are listed in `.github/pip-audit-ignore.txt` with a reason and expiry. Computer image is **not** built in Actions. Attestations apply to Releases after this landed. |
