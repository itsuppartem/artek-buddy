...
| Owner takeover vs helper input | Owner signs in while the helper still clicks or shells into the guest | Host `ComputerService` + supervisor act/exec | While `control_holder=user` with a live lease, helper act/exec/open/launch/close are refused before supervisor I/O; owner input still requires the lease id. Observe stays view-only. | Stale queued actions after navigation/release ([#592](https://github.com/itsuppartem/artek-buddy/issues/592) / [#591](https://github.com/itsuppartem/artek-buddy/issues/591)). Saved evidence vs live preview is not split in history yet ([#592](https://github.com/itsuppartem/artek-buddy/issues/592)). |
| Owner `$HOME` | Read/write/exec outside the home | Paired `.deb` | `inspect_owner_path` jail under the owner home; write/exec consent per bot **and answering device**. Always from one paired window does not skip the card on another. Lookup with no turn device matches only host-wide grants (`device_id` NULL), not any device's row. `git` and `find` are mutating unless they match an inspect-only argv (no `--output`, config, or worktree-changing flags; `git branch` is list-only). `rg`/`grep` are mutating unless argv cannot name a preprocessor, hostname helper, or config file; a path-form binary or leading `VAR=value` is not explore-only. The paired `.deb` refuses git/find file-output targets outside `$HOME` before `shell=True`. | Jail checks the logical path and then `resolve()`; symlinks escaping the owner home are denied. CodeQL `py/path-injection` on that join is accepted residual, not a forgotten traversal. Bind mounts on the owner PC remain a residual. Auto owner-read of files still happens without a card. Command strings after Allow are still a shell; that residual is named below. A worker started from that Deb turn inherits the Deb device so Always still covers its This-PC jobs. Inbox follow-ups without a stored actor fail closed (host-wide grants only). |
...
| Owner Send `command_id` | Two windows or a lost POST dispatch two executors for one command | Host HTTP + Postgres | Acceptance returns created/queued/replayed/resumed; a pending `turn_dispatches` row is claimed once before `_run_turn`; a changed payload is 409 without dispatch; Stop cancels the registered task; a queued command retry does not claim the blocking run; inbox claim rebinds `owner_commands.run_id` to the follow-up; no default model still records `needs_setup` and dedupes retry | Crash after claim and before the asyncio task starts is the same recovery card as a mid-flight restart. |
...
5. The paired `.deb` can run a shell on the owner PC (`/local/owner-exec`, loopback + this-window Origin + process nonce, `$HOME` jail). Mutating `git`/`find` (file output, create/rename branch) and search tools that can run another program (`rg --pre`, `--hostname-bin`, `--config`) require Allow; Deny does not run them. Search argv is parsed with option arity so `--` as a `-e`/`--regexp`/`-f` value cannot hide later executing flags; unknown search flags require Allow. A git/find write path outside `$HOME` is refused even if the host classifier is wrong. That is the owner, not the sandbox. CodeQL `py/command-line-injection` on that line is accepted residual risk, not a forgotten `shell=True`.
