...
| Loopback `/local/*` RPC | Another page on this PC calls owner-exec / pair / unpair | Paired `.deb` HTTP | Required Origin matching the window; Host; reject `Sec-Fetch-Site: cross-site`; JSON Content-Type; size cap; process nonce (`hmac.compare_digest`) | A process on the owner PC that can read the nonce (or control the window) can still call `/local/*`. `shell=True` owner-exec remains. |
