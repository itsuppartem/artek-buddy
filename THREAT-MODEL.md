...
remains open** from this page. Residual risk is named on purpose. Sandbox
CapDrop / non-root is [issue #88](https://github.com/itsuppartem/artek-buddy/issues/88),
not this document.
...
| Untrusted desktop | Agent breakout, lateral movement, noisy neighbors | Container + `artek-computers` network | `no-new-privileges`; `enable_icc=false`; noVNC published on `127.0.0.1` only; capability consent in the thread | **No `USER`, no CapDrop, no memory/CPU/pids, no read-only root.** Chromium/X11 image is still root in the box. That gap is #88. |
...
| Desktop breakout | Escape to the Pi or another bot | Engine + network | Isolated compose network, ICC off, host token never in the page | Root in the desktop, Docker socket on the supervisor, shared Team home. |
...
3. Desktop boxes are not a hardened jail until #88.
