...
| Owner `$HOME` | Read/write/exec outside the home | Paired `.deb` | `inspect_owner_path` jail under the owner home; write/exec consent per bot. `git` and `find` are mutating unless they match an inspect-only argv (no `--output`, config, or worktree-changing flags; `git branch` is list-only). The paired `.deb` refuses git/find file-output targets outside `$HOME` before `shell=True`. | Jail is the **logical** path then `resolve()`. Symlinks and bind mounts on the owner PC are a residual. Auto owner-read of files still happens without a card. Command strings after Allow are still a shell; that residual is named below. |
