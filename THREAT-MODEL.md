...
| Untrusted desktop | Agent breakout, lateral movement, noisy neighbors | Container + `artek-computers` network | uid `1000`; `CapDrop: ALL`; `no-new-privileges`; 1536 MiB / 1 CPU / 512 pids; tmpfs `/tmp`; `enable_icc=false`; noVNC on `127.0.0.1`; capability consent in the thread | Chromium starts with `--no-sandbox` (user namespaces in Docker). Rootfs is writable. No gVisor. Bind home is chowned to uid 1000. |
...
| Desktop breakout | Escape to the Pi or another bot | Engine + network | Isolated compose network, ICC off, non-root + CapDrop ALL, host token never in the page | `--no-sandbox` Chromium, writable rootfs, Docker socket on the supervisor, shared Team home. |
...
3. Desktop boxes drop all capabilities and run as uid 1000 with Pi 5-sized memory/CPU/pids limits. They are not a kernel jail: Chromium is `--no-sandbox`, the rootfs is writable, and there is no gVisor.
