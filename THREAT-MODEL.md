...
| Per-turn token usage | Stolen token sees spend counts; a log line repeats a prompt | Host HTTP + Postgres `usage_records` | Same GET auth as artifacts (missing bearer 401, bad token 403). Rows and `usage.recorded` SSE are integer counts plus bot/run ids. Missing usage is omitted. | Counts are not secrets. A stolen token sees spend. Prompts and keys stay out of this table and event. |
