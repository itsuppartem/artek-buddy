...
5. The paired `.deb` can run a shell on the owner PC (`/local/owner-exec`, loopback + this-window Origin + process nonce, `$HOME` jail). Mutating `git`/`find` (file output, create/rename branch) and search tools that can run another program (`rg --pre`, `--hostname-bin`, `--config`) require Allow; Deny does not run them. A git/find write path outside `$HOME` is refused even if the host classifier is wrong. That is the owner, not the sandbox. CodeQL `py/command-line-injection` on that line is accepted residual risk, not a forgotten `shell=True`.
