...
| Owner Send `command_id` | Two windows or a lost POST dispatch two executors for one command | Host HTTP + Postgres | Acceptance returns created/queued/replayed; a pending `turn_dispatches` row is claimed once before `_run_turn`; a changed payload is 409 without dispatch; Stop cancels the registered task; a queued command retry does not claim the blocking run; inbox claim rebinds `owner_commands.run_id` to the follow-up | Crash after claim and before the asyncio task starts is the same recovery card as a mid-flight restart. |
