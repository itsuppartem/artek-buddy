...
| Bot GitHub/PyPI tokens | Stolen file under `/data/credentials`, a chat paste, computer Reset leftover, or a log/resume line | Host API process + `/data` bind | Files are `0600` under `/data/credentials/<bot_id>/`, not `/root` and not `/data/homes`. `bot_id` is `bot_` + 16 hex, provider is `github`/`pypi`, then `contained_under`. HTTP returns last four only; chat/memory/answer paste of known token shapes is 400 before persist; Forget and bot delete unlink the files; Reset and Team ↔ Private do not; `observe` redacts stored values | A process that can read `/data` on the Pi owns those tokens. The store is plaintext files on the volume (no external vault). CodeQL `py/clear-text-storage-sensitive-data` on that write and `py/path-injection` on the jailed join are accepted residuals, not forgotten holes. The local Cursor shell does not receive them via process-wide env (that would leak across bots). Tokens typed in chat before this gate should be rotated. |
